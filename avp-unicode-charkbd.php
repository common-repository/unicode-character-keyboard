...
/*
  ...
  Plugin URI: http://www.terryobrien.me/programming/wordpress/plugins/unicode-character-keyboard/
...
/*
  Copyright 2012, 2015 Terry O'Brien
...
class avp_unicode_charkbd
{
    ...
    public function __construct()
    {
        ...
        register_deactivation_hook( __FILE__, array( &$this, 'avp_unicode_charkbd_admin_deactivate' ) );
         ...
         */
        wp_register_script( 'avp-unicode-charkbd-character', plugins_url( 'js/avp-unicode-charkbd-character-min.js', __FILE__ ), array( 'jquery', 'jquery-validate', 'jquery-validate-additional' ) );
        wp_register_script( 'avp-unicode-charkbd-file', plugins_url( 'js/avp-unicode-charkbd-file-min.js', __FILE__ ), array( 'jquery', 'jquery-validate', 'jquery-validate-additional' ) );
        wp_register_script( 'avp-unicode-charkbd-menu', plugins_url( 'js/avp-unicode-charkbd-menu-min.js', __FILE__ ), array( 'jquery' ) );
        wp_register_script( 'avp-unicode-charkbd-post', plugins_url( 'js/avp-unicode-charkbd-post-min.js', __FILE__ ), array( 'jquery' ) );
        wp_register_script( 'avp-unicode-charkbd-postbox', plugins_url( 'js/avp-unicode-charkbd-postbox-min.js', __FILE__ ), array( 'jquery', 'postbox' ) );

        wp_register_style( 'avp-unicode-charkbd-abbr', plugins_url( 'css/avp-unicode-charkbd-abbr-min.css', __FILE__ ) );
        wp_register_style( 'avp-unicode-charkbd-evenodd', plugins_url( 'css/avp-unicode-charkbd-evenodd-min.css', __FILE__ ) );
        wp_register_style( 'avp-unicode-charkbd-highlight', plugins_url( 'css/avp-unicode-charkbd-highlight-min.css', __FILE__ ) );
        wp_register_style( 'avp-unicode-charkbd-metabox', plugins_url( 'css/avp-unicode-charkbd-metabox-min.css', __FILE__ ) );

        /*
         *  Set up jQuery validation module
         */
        wp_register_script( 'jquery-validate', plugins_url( 'js/jquery.validate.min.js', __FILE__ ), array( 'jquery' ), '1.14.0' );
        wp_register_script( 'jquery-validate-additional', plugins_url( 'js/additional-methods.min.js', __FILE__ ), array( 'jquery', 'jquery-validate' ), '1.14.0' );
    ...
    function avp_unicode_charkbd_admin_init()
    {
        ...
        {
            if ( ( 'avp-unicode-charkbd-menu' === $_REQUEST[ 'page' ] ) )
            {
    ...
    function avp_unicode_charkbd_admin_menu_admin()
    {
        ...
        for ( $typeindex = 0, $linecount = 0; ( $typeindex < count( $this->characterSetTypes ) ); $typeindex += 1, $linecount = 0 )
        {
             ...
             */
            echo( '<table class="form-table nav-tab-contents" id="avp_unicode_charkbd_admin_menu_panel_' . $typeindex . '"' . ' style="display:' . ( ( 0 == $typeindex ) ? '' : 'none;' ) . '" >' );
            ?>
    ...
    function avp_unicode_charkbd_admin_menu_manage()
    {
        ...
        if ( ( isset( $_REQUEST[ 'action' ] ) ) && ( 'upload' == $_REQUEST[ 'action' ] ) )
        {
            ...
            if ( 'text/xml' !== $_FILES[ 'uploadFile' ][ 'type' ] )
            {
        ...
        else
        {
            ...
            <div id="poststuff">
                <div class="postbox" >
                    ...
                    <div class="inside">
                        <pre>
                            <?php echo( $this->avp_unicode_charkbd_return_file_contents( plugin_dir_path( __FILE__ ) . '/error.log' ) ); ?>
                        </pre>
                    </div>
                </div>
            </div>
            ...
            <form method="post" enctype="multipart/form-data" >
                ...
                <label for="delete">
                    <?php echo( '<input type="submit" name="delete" class="button-primary" value="Delete" onClick="return confirm( \'' . __( 'Are you sure you want to delete the log file?', 'avp-unicode-charkbd' ) . '\' );" />' ); ?>
                    <?php _e( '<strong>To delete the error log file, click the "Delete" button.</strong>', 'avp-unicode-charkbd' ); ?>
                </label>
            </form>
        ...
        <p>
        <form method="post" enctype="multipart/form-data" id="findCharacter" >
            <?php echo wp_nonce_field( 'manage-find' ); ?>
            <input type="hidden" name="action" value="find" />
            <label for="find">
                <input type="submit" name="find" class="button-primary" value="Find" />
                <input type="text" maxlength="8" size="8" name="character" id="character" >
            </label>
        </form>
        </p>
    ...
    {
        printf( '<h2>' );
        printf( '<img src="' . plugins_url( ( 'images/keyboard-32.png' ), __FILE__ ) . '" />' );
        printf( ' %s - %s</h2>', $this->title, $pagename );
    }
     ...
     *  'success' => 'updated', 'error' => 'error', 'warning' => 'update-nag'
     *  https://premium.wpmudev.org/blog/adding-admin-notices/
     */

    function avp_unicode_charkbd_admin_menu_message( $type, $message, $information = null )
    {
        $message_type = array( 'success' => 'updated', 'error' => 'error', 'warning' => 'update-nag' );

        echo( '<div id="message" class="' . $message_type[ $type ] . ' notice below-h2 is-dismissible" >' );
        echo( '<p>' );
        echo( '<img src="' . plugins_url( ( 'images/' . $type . '-32.png' ), __FILE__ ) . '" height="32" width="32" style="float:left; margin: 5px;" />' );
        echo( $message );
        echo( '<br />' );
        echo( ( ! is_null( $information ) ) ? $information : '&nbsp;' );
    ...
    function avp_unicode_charkbd_admin_menu_tablist( $tabNames, $tabNumber = 0 )
    {
        ...
        for ( $tabindex = 0; ( $tabindex < count( $tabNames ) ); ++ $tabindex )
        {
